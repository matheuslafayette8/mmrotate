...
engine_file = '/persistent/models/04_11_all_india.engine'
lib_path = '/persistent/models/mmdeploy_plugin/libmmdeploy_tensorrt_ops.so'
...
def get_bounding_box(image, output_list, threshold):
    ...
    for idx, entry in enumerate(output_list):
        ...
        output.append({
            ...
            "angle": angle,
            "score": score,
            "points": box  # Convert NumPy array to list for JSON serialization
        })
...
def main():
    folder_path_out = "/persistent/datasets/04_11_all_images/results_txt/"
    ...
    folder_path_in = "/home/luminar/mmrotate/dataset/04_11_all_india/imgs/"
    ...
    with load_engine(engine_file) as engine:
        for file in files:
            if file.endswith(".png"):
                ...
                new_name = os.path.splitext(file)[0] + "_out.txt"
                output_path = os.path.join(folder_path_out, new_name)
                
                with open(output_path, 'w') as f:
                    for box in boxes:
                        f.write(f"bouding box {box['bouding box']}\n")
                        f.write(f"cx = {box['cx']:.2f}\n")
                        f.write(f"cy = {box['cy']:.2f}\n")
                        f.write(f"width = {box['width']:.2f}\n")
                        f.write(f"length = {box['length']:.2f}\n")
                        f.write(f"angle = {box['angle']:.2f}\n")
                        f.write(f"score = {box['score']:.2f}\n")
                        f.write(f"points = {box['points']}\n\n")
